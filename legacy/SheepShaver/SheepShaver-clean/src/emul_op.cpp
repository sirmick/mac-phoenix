...
static int post1462_log = 0;
static bool past_cl1462 = false;
void EmulOp(M68kRegisters *r, uint32 pc, int selector)
{
	D(bug("EmulOp %04x at %08x\n", selector, pc));
	if (past_cl1462 && post1462_log < 200) {
		post1462_log++;
		fprintf(stderr, "[POST-1462] #%d sel=%d pc=%08x d0=%08x a7=%08x\n",
			post1462_log, selector, pc, r->d[0], r->a[7]);
	}
	switch (selector) {
...
//			D(bug("SCSIDispatch(%d)\n", sel));
		...
		case OP_CHECKLOAD: {		// vCheckLoad() patch
			static int cl_total_ss = 0;
			cl_total_ss++;
			uint32 type = ReadMacInt32(r->a[7]);
			r->a[7] += 4;
			int16 id = ReadMacInt16(r->a[2]);
			fprintf(stderr, "[CL] #%d '%c%c%c%c' id=%d\n",
				cl_total_ss,
				(type>>24)&0xff, (type>>16)&0xff, (type>>8)&0xff, type&0xff, id);
			if (r->a[0] == 0)
				break;
			...
			// Dump Mac state at CL=1462 for comparison with mac-phoenix
			{
				if (cl_total_ss == 1462) {
					const char *path = "/tmp/ss_ram_cl1462.bin";
					FILE *f = fopen(path, "wb");
					if (f) {
						uint32 dump_size = 0x200000;
						for (uint32 i = 0; i < dump_size; i++)
							fputc(ReadMacInt8(i), f);
						fclose(f);
						fprintf(stderr, "[SNAPSHOT] CL#1462: dumped %u bytes to %s\n", dump_size, path);
					}
					uint32 lv1 = ReadMacInt32(0x064);
					fprintf(stderr, "[SNAPSHOT] Lv1Int=%08x, code:", lv1);
					for (int j = 0; j < 32; j++)
						fprintf(stderr, " %04x", ReadMacInt16(lv1 + j*2));
					fprintf(stderr, "\n");
					fprintf(stderr, "[SNAPSHOT] XLM_RUN_MODE=%d XLM_68K_R25=%08x XLM_IRQ_NEST=%d\n",
						ReadMacInt32(XLM_RUN_MODE), ReadMacInt32(XLM_68K_R25), ReadMacInt32(XLM_IRQ_NEST));
					fprintf(stderr, "[SNAPSHOT] 68k: d0=%08x a0=%08x a7=%08x\n",
						r->d[0], r->a[0], r->a[7]);
					past_cl1462 = true;
				}
			}
