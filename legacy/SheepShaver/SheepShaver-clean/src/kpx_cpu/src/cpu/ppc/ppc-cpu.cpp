...
// Deterministic interrupt replay from legacy run
#include "hi_replay.h"
int hi_replay_idx = 0; // non-static so HandleInterrupt can read it
#ifndef SHEEPSHAVER
...
{
	use_jit = false; // DETERMINISTIC MODE: interpreter only
	if (cache_size)
...
void powerpc_cpu::execute(uint32 entry)
{
...
#endif
	execute_depth++;
	if (execute_depth >= 2 && (ppc_insn_counter == 127000000 || ppc_insn_counter == 128000000 || ppc_insn_counter == 129000000)) {
		static int depth_log = 0;
		if (++depth_log <= 200)
			fprintf(stderr, "[DEPTH] enter depth=%d insn=%llu entry=%08x\n", execute_depth, (unsigned long long)ppc_insn_counter, entry);
	}
#if PPC_DECODE_CACHE || PPC_ENABLE_JIT
  ...
  do_interpret:
	for (;;) {
		...
		ppc_insn_counter++;
		// Deterministic interrupt replay: DISABLED (debug aid only)
#if 0
		if (execute_depth == 1 && hi_replay_idx < hi_replay_count &&
		    ppc_insn_counter >= hi_replay_insn[hi_replay_idx]) {
			hi_replay_idx++;
			extern void SetInterruptFlag(uint32);
			SetInterruptFlag(1); // INTFLAG_VIA
  ...
  return_site:
	...
	if (execute_depth >= 2 && (ppc_insn_counter == 127000000 || ppc_insn_counter == 128000000 || ppc_insn_counter == 129000000)) {
		static int exit_log = 0;
		if (++exit_log <= 200)
			fprintf(stderr, "[DEPTH] exit  depth=%d insn=%llu\n", execute_depth, (unsigned long long)ppc_insn_counter);
	}
	--execute_depth;
}
