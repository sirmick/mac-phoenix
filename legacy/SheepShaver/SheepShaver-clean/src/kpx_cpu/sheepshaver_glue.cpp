...
void sheepshaver_cpu::interrupt(uint32 entry)
{
	...
	assert(gpr(6) != 0);
	{
		...
		static int int_log = 0;
		if (++int_log <= 200)
			fprintf(stderr, "[INT-SAVE] insn=%llu gpr6=%08x save_addr=%08x gpr7=%08x gpr8=%08x\n",
				(unsigned long long)ppc_insn_counter, (unsigned)gpr(6),
				(unsigned)(gpr(6) + 0x13c), (unsigned)gpr(7), (unsigned)gpr(8));
	}
...
void sheepshaver_cpu::execute_68k(uint32 entry, M68kRegisters *r)
{
	...
	// Dump memory at 0x6208
	{
		static bool dumped_6208 = false;
		if (false && !dumped_6208 && ppc_insn_counter >= 2622000) {
			dumped_6208 = true;
			fprintf(stderr, "[MEM-DUMP] @0x6208:");
			for (int i = 0; i < 32; i++)
				fprintf(stderr, " %02x", ReadMacInt8(0x6208 + i));
			fprintf(stderr, "\n");
			fprintf(stderr, "[MEM-DUMP] EmulData+0x808=%08x EmulData+0x804=%08x\n",
				ReadMacInt32(0x68fff808), ReadMacInt32(0x68fff804));
		}
	}

#if EMUL_TIME_STATS
	exec68k_count++;
	const clock_t exec68k_start_clk = clock();
#endif
	...
	// Log Execute68k duration
	{
		uint64_t e68k_insns = ppc_insn_counter - e68k_start;
		static int e68k_log = 0;
		if (e68k_start >= 14000000ULL && e68k_start <= 300000000ULL && ++e68k_log <= 500)
			fprintf(stderr, "[E68K] entry=%08x start=%llu dur=%llu\n",
				entry, (unsigned long long)e68k_start, (unsigned long long)e68k_insns);
	}

#if EMUL_TIME_STATS
	exec68k_time += (clock() - exec68k_start_clk);
#endif
}
...
void emul_ppc(uint32 entry)
{
	...
	ppc_cpu->start_log();
#endif
	// Pre-execute memory checksum for determinism verification
	{
		uint8 *ram = Mac2HostAddr(0);
		uint32 ram_cksum = 0, rom_cksum = 0, xlm_cksum = 0;
		for (uint32 i = 0; i < RAMSize && i < 0x4000000; i += 4)
			ram_cksum ^= *(uint32*)(ram + i);
		for (uint32 i = 0; i < 0x400000; i += 4)
			rom_cksum ^= *(uint32*)(Mac2HostAddr(ROMBase) + i);
		for (uint32 i = 0; i < 0x3000; i += 4)
			xlm_cksum ^= ReadMacInt32(i);
		fprintf(stderr, "[MEM-CHECK] RAM=%08x ROM=%08x XLM=%08x entry=%08x KD=%08x\n",
			ram_cksum, rom_cksum, xlm_cksum, entry, KernelDataAddr);
	}
...
void HandleInterrupt(powerpc_registers *r)
{
	...
	if (int32(ReadMacInt32(XLM_IRQ_NEST)) > 0) {
		static int nest_skip = 0;
		if (++nest_skip <= 5)
			fprintf(stderr, "[HI] skip: IRQ_NEST=%d\n", ReadMacInt32(XLM_IRQ_NEST));
		return;
	}
	...
	// Log HandleInterrupt progress
	if (hi_count <= 10 || (hi_count % 100 == 0)) {
		extern uint64_t ppc_insn_counter;
		fprintf(stderr, "[HI] #%d mode=%d insn=%llu\n",
			hi_count, mode, (unsigned long long)ppc_insn_counter);
	}
...
{
	// Track native op rates per 5s — same as mac-phoenix
	{
		...
		if (now - last_dump >= 5000000) {
			...
			fprintf(stderr, "[NOP-RATE] #%d: VBL=%d DIO=%d RES=%d CL=%d NCL=%d MKX=%d NQD=%d\n",
				dump_count,
				nop_counts[2],   // NATIVE_VIDEO_VBL
				nop_counts[3],   // NATIVE_VIDEO_DO_DRIVER_IO
				nop_counts[22]+nop_counts[23]+nop_counts[24]+nop_counts[25]+nop_counts[26]+nop_counts[29]+nop_counts[30],
				nop_counts[28],  // NATIVE_CHECK_LOAD_INVOC
				nop_counts[31],  // NATIVE_NAMED_CHECK_LOAD_INVOC
				nop_counts[27],  // NATIVE_MAKE_EXECUTABLE
				nop_counts[32]+nop_counts[33]+nop_counts[34]+nop_counts[35]+nop_counts[36]+nop_counts[37]);
	...
	case NATIVE_CHECK_LOAD_INVOC: {
		...
		++cl_count;
		if (cl_count <= 5 || cl_count % 200 == 0)
			fprintf(stderr, "[CHECKLOAD] #%d\n", cl_count);
		check_load_invoc(gpr(3), gpr(4), gpr(5));
		break;
	}
	case NATIVE_NAMED_CHECK_LOAD_INVOC: {
		static int ncl_count = 0;
		++ncl_count;
		if (ncl_count <= 5 || ncl_count % 200 == 0)
			fprintf(stderr, "[NAMED-CHECKLOAD] #%d\n", ncl_count);
