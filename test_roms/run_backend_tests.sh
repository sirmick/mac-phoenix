...
IRQ_COUNT=$(env EMULOP_VERBOSE=1 \
    ./build/mac-phoenix --backend uae --timeout $TIMEOUT --rom test_roms/test_irq_uae.rom --no-webserver 2>&1 | \
...
ALINE_COUNT=$(env EMULOP_VERBOSE=1 \
    ./build/mac-phoenix --backend unicorn --timeout $TIMEOUT --rom test_roms/test_irq_unicorn.rom --no-webserver 2>&1 | \
