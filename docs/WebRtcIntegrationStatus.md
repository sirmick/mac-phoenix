# WebRTC Integration Status - mac-phoenix
...
mac-phoenix uses an **in-process WebRTC architecture** - unlike the split-process IPC design of web-streaming, all components (CPU emulation, video/audio encoding, WebRTC signaling, HTTP server) run in a single process. This eliminates IPC overhead and simplifies deployment.
...
- **Required workflow:** Start with ROM path: `./build/mac-phoenix <rom-path>`

**Web UI configuration works, but:**
- User can configure ROM in web UI
- Config is saved to `~/.config/mac-phoenix/config.json`
- But process must be **restarted** with ROM path for CPU to initialize

**Example:**
```bash
# Start with ROM directly
./build/mac-phoenix "/home/mick/macemu/web-streaming/storage/roms/1MB ROMs/1991-10 - 420DBFF3 - Quadra 700&900 & PB140&170.ROM"
...
### mac-phoenix Config
**Location:** `~/.config/mac-phoenix/config.json`
...
2. **Run with ROM:**
   ```bash
   ./build/mac-phoenix "/path/to/rom.ROM"
   ```
