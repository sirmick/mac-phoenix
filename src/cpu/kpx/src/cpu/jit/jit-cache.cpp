...
basic_jit_cache::init_translation_cache(uint32 size)
{
	...
	// Pad allocator to match legacy SheepShaver's JIT code cache address
	// (0x10402000). Legacy has ~3MB of block cache tables between the decode
	// cache and JIT code buffer. Without this padding, the JIT lands at a
	// different x86 address, which changes JIT block chaining timing and
	// breaks the MODE_NATIVE positive feedback loop during PPC boot.
	{
		void *cur = vm_get_next_address();
		void *target = (void *)0x10402000;
		if (cur < target) {
			size_t pad = (char *)target - (char *)cur;
			vm_acquire(pad);  // dummy allocation to advance next_address
		}
	}
