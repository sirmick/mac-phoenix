...
{
	// Categorize illegal instructions to understand what's being skipped
	static int illegal_count = 0;
	uint32 primary = (opcode >> 26) & 0x3f;
	if (++illegal_count <= 50 || primary == 3 || illegal_count % 10000 == 0)
		fprintf(stderr, "[ILLEGAL] #%d pc=%08x opcode=%08x primary=%d%s\n",
			illegal_count, pc(), opcode, primary,
			primary == 3 ? " *** TWI (KCall trap!) ***" : "");
