/*
 *  video_ppc.cpp - PPC Video/graphics emulation
 *
 *  Lifted from SheepShaver's video.cpp with platform-specific
 *  WebRTC video output appended at the end.
 *
...
// IPC video hooks (video_ipc_ppc.cpp, extern "C")
extern "C" void video_ipc_set_framebuffer(const uint8_t* fb);
extern "C" void video_ipc_set_resolution(int width, int height);
extern "C" void video_ipc_refresh(void);

// ADB stub — PPC doesn't use this from video
static inline void ADBMouseMoved(int, int) {}
...
void video_set_cursor(void)
{
	...
	// IPC: cursor position tracked via SHM, no bitmap support yet
	// Same as legacy IPC — cursor rendered browser-side
}

void video_set_dirty_area(int x, int y, int w, int h)
{
	(void)x; (void)y; (void)w; (void)h;
	// IPC: entire frame refreshed every tick, no dirty rect tracking
	// Same as legacy IPC
}
...
struct VideoInfo ppc::VModes[64];
...
VidLocals *ppc::private_data = NULL;	// Pointer to driver local variables (there is only one display, so this is ok)
...
bool ppc::VideoSnapshot(int xsize, int ysize, uint8 *p)
{
...
// ============================================================================
// Platform-specific: WebRTC video output for mac-phoenix
// ============================================================================
...
namespace video {
	extern VideoOutput* g_video_output;
...
static config::EmulatorConfig *g_ppc_video_config = nullptr;


/*
 *  video_mode_change - Handle video mode change request from Mac OS
 *  Writes framebuffer address (screen_base) into the parameter block
 *  so Mac OS knows where to draw.
 *  (Lifted from SheepShaver IPC video_ipc_sheep.cpp)
 */

int16 ppc::video_mode_change(VidLocals *csSave, uint32 ParamPtr)
{
	D(bug("video_mode_change: mode=%04x data=%08x\n",
	      ReadMacInt16(ParamPtr + csMode), ReadMacInt32(ParamPtr + csData)));
...
/*
 *  VideoInit - Initialize PPC video subsystem
 *  Allocates framebuffer and sets Mac low memory globals.
 */

bool ppc::VideoInit(void)
{
	D(bug("VideoInit (PPC)\n"));

	// Init variables — copied from legacy IPC video driver
	private_data = NULL;
...
/*
 *  VideoExit - Clean up video subsystem
 */

void ppc::VideoExit(void)
{
	D(bug("VideoExit\n"));
	...
	if (the_buffer && the_buffer != MAP_FAILED) {
...
/*
 *  VideoVBL - Called from 60Hz interrupt
 *  Triggers VBL interrupt service if video driver has initialized.
 */

void ppc::VideoVBL(void)
{
...
// VideoInstallAccel: real implementation in gfxaccel_ppc.cpp


/*
 *  video_ppc_init_webrtc - Initialize WebRTC video pipeline for PPC
 *  Creates VideoOutput and launches encoder thread.
 *  Called after VideoInit() during PPC boot setup.
 */

static void video_ppc_init_webrtc_impl(config::EmulatorConfig *config);

void video_ppc_init_webrtc_null(void)
{
	video_ppc_init_webrtc_impl(nullptr);
}

void video_ppc_init_webrtc(config::EmulatorConfig *config)
{
	video_ppc_init_webrtc_impl(config);
}

// Forward declaration
void video_ppc_refresh(void);

static void video_ppc_init_webrtc_impl(config::EmulatorConfig *config)
{
	g_ppc_video_config = config;

	// In IPC child mode, video_refresh is set to video_ipc_refresh
	// which writes frames to SHM for the parent to relay. Don't override it.
	extern Platform g_platform;
	if (g_platform.video_refresh == video_ipc_refresh) {
		fprintf(stderr, "[PPC-Video] IPC mode detected, skipping WebRTC video init\n");
		return;
	}

	// Set PPC video refresh (headless / non-IPC mode)
	g_platform.video_refresh = video_ppc_refresh;
...
/*
 *  video_ppc_refresh - Capture frame and submit to VideoOutput
 *  Called periodically from the tick thread or main loop.
 */
...
void video_ppc_refresh(void)
{
	...
	// Mac framebuffer is big-endian ARGB: bytes in memory are [A,R,G,B]
	// PIXFMT_ARGB tells the encoder to extract R=byte[1], G=byte[2], B=byte[3]
	const uint32_t *pixels = reinterpret_cast<const uint32_t *>(the_buffer);
	video::g_video_output->submit_frame(pixels, frame_width, frame_height, PIXFMT_ARGB);
}


// NQD hooks: real implementations in gfxaccel_ppc.cpp


/*
 *  Palette/gamma — copied from legacy IPC video driver
 */

static uint32 current_palette[256 * 3];  // RGB triplets for indexed modes

void ppc::video_set_palette(void)
{
...
void ppc::video_set_gamma(int gamma)
{
	D(bug("video_set_gamma(%d)\n", gamma));
	(void)gamma;
	// No gamma correction in IPC mode — same as legacy
}
