...
//		case 80:	// 860
	...
	*lp = htonl(POWERPC_NOP);
	...
	// (Original SheepShaver approach: NOP the stwx and tblie inside the loop,
	// let the page table walk code run — it works because 0xdead0000 is
	// accessible in REAL_ADDRESSING mode)
...
static bool patch_nanokernel(void)
{
	...
	fprintf(stderr, "[ROMPATCH] ppc_excp_tbl at %08lx -> li r8,%d / stw r8,0x%x\n",
		(unsigned long)base, MODE_NATIVE, XLM_RUN_MODE);
	...
	*wp++ = htons((ROMBase + get_scrap) & 0xffff);
	...
	if (PrefsFindBool("idlewait")) {
		...
		if (ntohs(*wp) == 0x2078) {								// movea.l	ExpandMem,a0
			...
			*wp = htons(M68K_NOP);
			fprintf(stderr, "[ROMPATCH] SynchIdleTime → IDLE_TIME\n");
		}
		else if (ntohs(*wp) == 0x70fe) {						// moveq	#-2,d0
			*wp++ = htons(M68K_EMUL_OP_IDLE_TIME_2);
			fprintf(stderr, "[ROMPATCH] SynchIdleTime → IDLE_TIME_2\n");
		}
		else {
			fprintf(stderr, "[ROMPATCH] SynchIdleTime patch FAILED (unknown opcode %04x)\n", ntohs(*wp));
		}
	} else {
		fprintf(stderr, "[ROMPATCH] SynchIdleTime skipped (idlewait=false)\n");
	}
...
void ppc::InstallDrivers(void)
{
	fprintf(stderr, "[DRV] InstallDrivers() starting...\n");
	M68kRegisters r;
	...
	// Open .Sony driver
	fprintf(stderr, "[DRV] Opening .Sony...\n");
	...
	fprintf(stderr, "[DRV] .Sony opened\n");
	// Install disk driver
	...
	if (!PrefsFindBool("nocdrom")) {
		...
		// Open CD-ROM driver
		fprintf(stderr, "[DRV] Opening .AppleCD...\n");
	...
	fprintf(stderr, "[DRV] CD-ROM done, installing serial drivers...\n");
	// Install serial drivers
	...
	WriteMacInt16(dce + dCtlFlags, 0x4e00);
	fprintf(stderr, "[DRV] InstallDrivers() complete\n");
}
