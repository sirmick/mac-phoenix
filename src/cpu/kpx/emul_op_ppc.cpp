...
void ppc::EmulOp(M68kRegisters *r, uint32 pc, int selector)
{
	D(bug("EmulOp %04x at %08x\n", selector, pc));

	// Track boot progress via platform-level events (mac-phoenix web UI)
	{
...
//			D(bug("SCSIDispatch(%d)\n", sel));
		...
		case OP_NTRB_17_PATCH4:
			...
			if (ReadMacInt16(r->a[2]) == 11 && ReadMacInt16(r->a[2] + 6) == 17) {
				PatchNativeResourceManager();
				// mac-phoenix: force PatchAfterStartup since accRun never fires
				PatchAfterStartup_PPC();
			}
			break;
