...
// avoid having it show up for signals that get handled.
	...
	mapAddr = 0;
#endif
	fprintf(stderr, "[VM] vm_init: MAP_BASE=0x%lx next_address=%p\n",
		(unsigned long)MAP_BASE, (void*)next_address);
	return 0;
}
...
// Exposed so SheepMem::Init can align allocations to match legacy layout
void vm_set_next_address(void *addr) {
    next_address = (char *)addr;
}

void *vm_get_next_address(void) {
    return next_address;
}
