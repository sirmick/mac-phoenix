...
// Use EMULATED_PPC code paths (we use Unicorn, not real hardware)
...
// Global variables
int PPCROMType;				// ROM type
static uint32 sony_offset;	// Offset of .Sony driver resource

// PPC-specific globals
uint32 KernelDataAddr;		// Mac address of Kernel Data
uint32 ROMBase;				// Mac address of ROM base (= ROMBaseMac)
uint32 RAMBase;				// Mac address of RAM base (= 0)
uint32 PVR;					// Processor Version Register value
uint32 CPUClockSpeed;		// CPU clock in Hz
uint32 BusClockSpeed;		// Bus clock in Hz

// Extern globals from cpu_context.cpp / legacy code
...
// Alias ROMType for legacy code compatibility within this file
#define ROMType PPCROMType
...
bool DecodeROM_PPC(uint8 *data, uint32 size)
{
	if (size == PPC_ROM_SIZE) {
		// Plain ROM image
		memcpy(ROMBaseHost, data, PPC_ROM_SIZE);
		return true;
	}
...
// Forward declaration
static bool FindSifter(uint32 type, int16 id);

static void AddSifter(uint32 type, int16 id)
{
...
// TODO: Serial driver generation requires thunks system (NativeTVECT).
// Stubbed out until PPC thunks are implemented.
#if 0
...
#endif // Serial driver generation (requires thunks)
...
{
	// Initialize PPC globals
	ROMBase = ROMBaseMac;
	RAMBase = RAMBaseMac;
	KernelDataAddr = KERNEL_DATA_BASE;
	PVR = PPC_PVR_750_V3_1;
	CPUClockSpeed = PPC_CPU_CLOCK_HZ;
	BusClockSpeed = PPC_BUS_CLOCK_HZ;
	...
	*lp = htonl(0);
	...
	memcpy(ROMBaseHost + PPC_ROM_SIZE, ROMBaseHost + (PPC_ROM_SIZE - 0x100000), 0x100000);
	return true;
}
...
//	else
	...
	*wp = htons(M68K_RTS);
	...
	for (;;) {
		...
		while ((ntohl(*lp) & 0xff000000) == 0xff000000) {
			*lp = htonl((ntohl(*lp) & (PPC_ROM_SIZE-1)) + ROMBase);
			lp++;
		}
		while (!ntohl(*lp)) lp++;
	...
	*wp = htons(M68K_RTS);
	...
	// TODO: Install serial drivers (requires thunks/NativeTVECT)
	...
	// Copy icons to ROM
	// TODO: Icon data arrays need to be provided
	...
	*wp++ = htons((ROMBase + get_scrap) & 0xffff);
	...
	// Patch SynchIdleTime()
	if (true) {  // Always enable idle wait (was: PrefsFindBool("idlewait"))
...
void InstallDrivers_PPC(void)
{
	D(bug("PPC InstallDrivers: stubbed (requires Execute68k/thunks)\n"));
	// TODO: Full driver installation requires Execute68k(), Execute68kTrap(),
	// and SheepMem allocation. These will be implemented with the PPC EmulOp
	// dispatch system. For now, this is a no-op stub.
#if 0
	...
	WriteMacInt16(dce + dCtlFlags, 0x4e00);
#endif // InstallDrivers stub
}


/*
 *  Initialize XLM globals in low memory (0x2800..)
 */

void InitXLM(void)
{
	D(bug("Initializing XLM globals...\n"));

	// Clear XLM area
	memset(RAMBaseHost + XLM_SIGNATURE, 0, 0x100);
	...
	// Run mode = 68k initially
	WriteMacInt32(XLM_RUN_MODE, MODE_68K);

	// IRQ nesting = 1 (disabled initially)
	...
	// Create BootGlobs at top of Mac memory (like SheepShaver main.cpp)
	...
	D(bug("BootGlobs at %08x: MemTop=%08x RAM=%08x Size=%08x\n",
		  BootGlobsAddr, RAMBase + RAMSize, RAMBase, RAMSize));

	// Install SHEEP opcode trampolines for EMUL_RETURN, EXEC_RETURN, EMUL_OP
	// These are small PPC code sequences in low memory that the ROM's 68k
	// emulator jumps to. They contain SHEEP opcodes that Unicorn intercepts.
	// Place at 0x2900 (after XLM area, within low memory)
	uint32 tramp_base = 0x2900;

	// EMUL_RETURN trampoline: SHEEP opcode 0x18000000
	WriteMacInt32(tramp_base + 0, POWERPC_EMUL_OP | 0);  // SHEEP EMUL_RETURN
	WriteMacInt32(XLM_EMUL_RETURN_PROC, tramp_base);

	// EXEC_RETURN trampoline: SHEEP opcode 0x18000001
	WriteMacInt32(tramp_base + 4, POWERPC_EMUL_OP | 1);  // SHEEP EXEC_RETURN
	WriteMacInt32(XLM_EXEC_RETURN_PROC, tramp_base + 4);

	// EMUL_OP trampoline: SHEEP opcode 0x18000003+ (selector encoded in opcode)
	// The ROM patches install per-selector dispatch at ROM+0x380000,
	// but this pointer is used as a generic entry. Point to a blr for now.
	WriteMacInt32(tramp_base + 8, POWERPC_BLR);
	WriteMacInt32(XLM_EMUL_OP_PROC, tramp_base + 8);

	// EMUL_RETURN stack (top of RAM, like SheepShaver)
	WriteMacInt32(XLM_EMUL_RETURN_STACK, RAMBase + RAMSize - 0x100);

	// EXEC_RETURN opcode (used by Execute68k)
	WriteMacInt32(XLM_EXEC_RETURN_OPCODE, M68K_EXEC_RETURN);

	D(bug("XLM initialized: KernelData=%08x PVR=%08x BusClock=%u\n",
		  KernelDataAddr, PVR, BusClockSpeed));
	D(bug("XLM trampolines at %08x: EMUL_RETURN=%08x EXEC_RETURN=%08x\n",
		  tramp_base, tramp_base, tramp_base + 4));
}
