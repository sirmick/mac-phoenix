...
// PPC virtual clock: set to true by KPX backend init so timer_current_time
// uses instruction-counter-based time instead of wall clock, matching legacy.
bool g_use_ppc_virtual_clock = false;
...
void timer_current_time(tm_time_t &t)
{
	if (g_use_ppc_virtual_clock) {
		...
		t.tv_sec = virtual_ns / 1000000000ULL;
		t.tv_nsec = virtual_ns % 1000000000ULL;
		return;
	}
#if defined(__MACH__)
	mach_current_time(t);
...
#ifdef SHEEPSHAVER
	// PPC mode: no-op. Without SIGUSR2, blocking freezes the PPC thread.
	return;
#endif
